...
export class ApiHealthChecker {
  ...
  async performHealthCheck(endpoints: string[]): Promise<HealthCheckResult> {
    ...
    const validatedEndpoints = await this.validator.validateAllEndpoints(endpoints);
    const endTime = Date.now();

    const available = Object.values(validatedEndpoints).filter(e => e.isAvailable);
    const unavailable = Object.values(validatedEndpoints).filter(e => !e.isAvailable);
    
    const avgResponseTime = available.length > 0 
      ? available.reduce((sum, e) => sum + (e.responseTime || 0), 0) / available.length
      : 0;
    ...
    const result: HealthCheckResult = {
      ...
      endpoints: validatedEndpoints,
      summary: this.generateSummary(available.length, unavailable.length, avgResponseTime),
    };

    // Log results
    console.log(`🏥 Health Check Complete (${endTime - startTime}ms):`);
    console.log(`   📊 ${result.availableEndpoints}/${result.totalEndpoints} endpoints available`);
    console.log(`   ⚡ Average response time: ${result.avgResponseTime}ms`);
    
    if (result.unavailableEndpoints > 0) {
      console.error(`   ❌ ${result.unavailableEndpoints} endpoints are down:`, 
  ...
  async quickHealthCheck(criticalEndpoints: string[]): Promise<boolean> {
    ...
    try {
      const results = await this.validator.validateAllEndpoints(criticalEndpoints);
      const allAvailable = Object.values(results).every(e => e.isAvailable);
      
      if (!allAvailable) {
        const failed = Object.values(results).filter(e => !e.isAvailable);
        console.error('❌ Critical endpoints are down:', failed.map(e => e.endpoint));
      }
   ...
   */
  startHealthMonitoring(endpoints: string[], intervalMinutes: number = 5): () => void {
   ...
   */
  private generateSummary(available: number, unavailable: number, avgTime: number): string {
    if (unavailable === 0) {
      return `✅ All systems operational (${Math.round(avgTime)}ms avg)`;
    } else if (unavailable < available) {
      return `⚠️ ${unavailable} endpoints down, ${available} working (${Math.round(avgTime)}ms avg)`;
    } else {
  ...
  async exportHealthReport(endpoints: string[]): Promise<string> {
    ...
    const report = {
      ...
      'Endpoint Details': Object.entries(result.endpoints).map(([endpoint, status]) => ({
        endpoint,
...
export async function checkEndpointsHealth(endpoints: string[]): Promise<HealthCheckResult> {
  const healthChecker = getApiHealthChecker();
  return healthChecker.performHealthCheck(endpoints);
}

export async function validateCriticalEndpoints(endpoints: string[]): Promise<boolean> {
  const healthChecker = getApiHealthChecker();
