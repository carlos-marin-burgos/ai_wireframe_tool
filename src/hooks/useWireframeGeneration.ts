...
const wireframeCache: Record<string, {
  html: string;
  timestamp: number;
  processingTime: number;
}> = {};
...
export const useWireframeGeneration = () => {
  ...
  const generateWireframe = useCallback(async (
    ...
    const useAI = description.length > 50 || 
    ...
    setError(null);
    setFallback(false);
    setLoadingStage(useAI ? 'Initializing AI model...' : 'Generating fast wireframe...');
    
    // Create a cache key with version for Microsoft Design Language update
    // DEVELOPMENT: Always skip cache by making key unique
    const cacheKey = `${description}-${theme}-${colorScheme}-${useAI}-DEVELOPMENT-NOCACHE-${Date.now()}`;
    ...
    try {
      ...
      const data = await api.post<WireframeResponse>(
        ...
        { description, theme, colorScheme, useAI },
        { 
      ...
      console.log('📥 API response received:', {
        ...
        title: data.html?.match(/<title>(.*?)<\/title>/)?.[1] || 'No title found'
      });
      ...
      if (htmlContent) {
        console.log('HTML content length after ensureString:', htmlContent.length);
      ...
      try {
        ...
        const { generateFallbackWireframe } = await import('../utils/fallbackWireframeGenerator');
