...
export function generateHeroHTML(options: HeroOptions = {}): string {
  ...
  const actionSection = showSearch 
    ...
    : `<div style="display: flex; gap: 8px;">
         ...
         ${showSecondaryButton ? `<button style="box-sizing: border-box; display: flex; flex-direction: row; justify-content: center; align-items: center; padding: 6px 14px; gap: 8px; width: 70px; height: 38px; border: 1px solid #8E8E8E; border-radius: 2px; background: transparent; cursor: pointer; font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, sans-serif; font-weight: 600; font-size: 16px; color: #161616;">
           ${secondaryCtaText}
         </button>` : ''}
       </div>`;
