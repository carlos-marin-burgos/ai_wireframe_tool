...
const SplitLayout: React.FC<SplitLayoutProps> = ({
  ...
  const handleAddPagesToWireframe = useCallback((pages: Array<{
    ...
    if (newPages.length > 0) {
      ...
      // Give user feedback
      addMessage('ai', `📄 Created ${newPages.length} new page${newPages.length > 1 ? 's' : ''} and switched to "${firstNewPage.name}". You can now click on buttons and links in your wireframe to set up navigation between pages!`);
    }
    ...
    if (newPageContent) {
      ...
      if (currentPage) {
        ...
        const placeholderHtml = `
          <div style="max-width: 800px; margin: 0 auto; padding: 40px 20px; font-family: 'Segoe UI', sans-serif;">
            <div style="text-align: center; padding: 60px 20px; background: #f8f9fa; border-radius: 8px; border: 2px dashed #e1e5e9;">
              <h1 style="color: #0078d4; margin: 0 0 16px 0; font-size: 28px;">📄 ${currentPage.name}</h1>
              ...
              <div style="display: flex; gap: 12px; justify-content: center; flex-wrap: wrap;">
                <button style="background: #0078d4; color: white; border: none; padding: 12px 24px; border-radius: 4px; cursor: pointer; font-weight: 600;">
                  Generate Content
                </button>
                <button style="background: #f3f2f1; color: #323130; border: 1px solid #e1dfdd; padding: 12px 24px; border-radius: 4px; cursor: pointer; font-weight: 600;">
                  Copy from Home
                </button>
              </div>
            </div>
      ...
      <div className="right-pane">
        {(htmlWireframe || wireframePages.length > 0) ? (
          ...
          <div className="ai-assistant-container">
            ...
            <LoadingOverlay
              isVisible={loading}
              message={loading ? "Creating your wireframe..." : ""}
            />
          </div>
        )}
      </div>
