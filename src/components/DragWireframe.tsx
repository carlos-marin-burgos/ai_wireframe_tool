...
function sanitizeHTML(html: string): string {
    ...
    try {
        ...
        // If it's a complete HTML document, extract styles and body content while preserving layout structure
        if (cleanHtml.includes('<html') && cleanHtml.includes('</html>')) {
            ...
            if (bodyMatch) {
                ...
                if (bodyContent.includes('main-container') ||
                    bodyContent.includes('sidebar-left') ||
                    bodyContent.includes('sidebar-right') ||
                    bodyContent.match(/class="[^"]*sidebar[^"]*"/)) {

                    // Keep the full body content to preserve flex layout structure
                    cleanHtml = styles + bodyContent;
                } else {
                    // For non-layout HTML, extract just the body content as before
                    cleanHtml = styles + bodyContent;
                }
            }
        }
