...
const InteractiveWireframeRenderer: React.FC<InteractiveWireframeRendererProps> = ({
  ...
  const processHtmlForRendering = useCallback((html: string): string => {
    ...
    // Clean up any unwanted characters or markdown artifacts
    let processedHtml = html.trim();
    processedHtml = processedHtml.replace(/^[0'"]+|[0'"]+$/g, '');
    processedHtml = processedHtml.replace(/^'''html\s*/gi, '');
    processedHtml = processedHtml.replace(/^```html\s*/gi, '');
    processedHtml = processedHtml.replace(/```\s*$/gi, '');
    processedHtml = processedHtml.trim();

    // Early return if no content after cleaning
