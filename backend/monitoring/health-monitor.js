...
class HealthMonitor {
  constructor() {
    this.apiEndpoint = 'https://func-designetica-vjib6nx2wh4a4.azurewebsites.net/api/generate-html-wireframe';
    this.healthEndpoint = 'https://func-designetica-vjib6nx2wh4a4.azurewebsites.net/api/health';
    this.websiteEndpoint = 'https://designetica.carlosmarin.net';
  ...
  async checkHealth() {
    ...
    try {
      ...
      results.checks.healthEndpoint = await this.checkEndpoint(this.healthEndpoint, 'GET');
      
      // Check 2: Main API functionality
      results.checks.apiEndpoint = await this.checkApiEndpoint();
      
      // Check 3: Website accessibility
      results.checks.website = await this.checkEndpoint(this.websiteEndpoint, 'GET');
      ...
      console.log(`✅ Health check completed. Status: ${results.overall.status}`);
      return results;
  ...
  async checkApiEndpoint() {
    ...
    const result = await this.checkEndpoint(this.apiEndpoint, 'POST', testPayload);
  ...
  async checkDnsResolution() {
    return new Promise((resolve) => {
      ...
      dns.lookup('designetica.carlosmarin.net', (err, address) => {
        const responseTime = Date.now() - startTime;
  ...
  async checkSslCertificate() {
    return new Promise((resolve) => {
      ...
      const socket = tls.connect(443, 'designetica.carlosmarin.net', () => {
        ...
        resolve({
          status: now >= validFrom && now <= validTo ? 'HEALTHY' : 'UNHEALTHY',
          validFrom: cert.valid_from,
          validTo: cert.valid_to,
          daysUntilExpiry,
          issuer: cert.issuer.CN,
          responseTime,
          warning: daysUntilExpiry < 30 ? `Certificate expires in ${daysUntilExpiry} days` : null
        });
      });
  ...
  async checkForAlerts(results) {
    ...
    if (this.lastStatus && this.lastStatus !== currentStatus) {
      await this.triggerAlert('STATUS_CHANGE', `Status changed from ${this.lastStatus} to ${currentStatus}`);
    }
    ...
    if (results.checks.ssl?.daysUntilExpiry < 30) {
      await this.triggerAlert('SSL_EXPIRY_WARNING', `SSL certificate expires in ${results.checks.ssl.daysUntilExpiry} days`);
    }
