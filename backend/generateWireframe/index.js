...
`;
  ...
  const basePrompt = `You are a Senior UX Engineer at Microsoft with expertise in creating production-ready, comprehensive wireframes that include ALL necessary components for the described functionality.

CRITICAL MISSION: Create a COMPLETE, ACCURATE wireframe that includes EVERY component needed for: "${description}"
...
function validateAndEnhanceWireframe(html, description, componentAnalysis) {
  ...
  if ((desc.includes('form') || desc.includes('input') || desc.includes('textbox')) && 
      !html.includes('fluent-text-field') && !html.includes('<input')) {
    console.log('⚠️ Form elements missing, adding basic form structure');
    
    // Add a form section before the closing main tag
    const formSection = '<section class="form-section" style="background: white; padding: 32px; border-radius: 8px; margin: 24px auto; max-width: 600px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">' +
      '<h2>Contact Form</h2>' +
  ...
  if ((desc.includes('button') || desc.includes('submit') || desc.includes('click')) && 
      !html.includes('fluent-button') && !html.includes('<button')) {
    console.log('⚠️ Button elements missing, adding action buttons');
    
    // Add buttons section
    const buttonSection = '<section class="actions-section" style="text-align: center; margin: 32px 0;">' +
    ...
    enhancedHtml = enhancedHtml.replace('</main>', buttonSection + '\n</main>');
  }
  
  // Check if dashboard/metrics are mentioned but no data visualization exists
  if ((desc.includes('dashboard') || desc.includes('metric') || desc.includes('chart') || desc.includes('analytics')) && 
      !html.includes('metric') && !html.includes('chart') && !html.includes('fluent-progress')) {
    console.log('⚠️ Dashboard components missing, adding metrics section');
    
    const metricsSection = '<section class="metrics-section" style="margin: 32px 0;">' +
    ...
    enhancedHtml = enhancedHtml.replace('</main>', metricsSection + '\n</main>');
  }
