...
app.post('/api/fast-suggestions', (req, res) => {
  ...
  console.log('🧠 AI Suggestions for:', userInput);
...
<style>body{font-family:'Segoe UI',sans-serif;margin:40px;background:#f5f5f5}.container{max-width:800px;margin:0 auto;background:white;padding:2rem;border-radius:8px}h1{color:#8E9AAF}</style>
</head><body><div class="container"><h1>✅ Working Wireframe</h1><p><strong>Description:</strong> ${description || 'No description'}</p><p>Generated by reliable Express server - no more Azure Functions issues!</p></div></body></html>`;
  
  res.json({ html, timestamp: new Date().toISOString(), generatedBy: 'ReliableExpress' });
});

app.listen(PORT, () => {
  console.log(`🎉 RELIABLE EXPRESS SERVER RUNNING ON http://localhost:${PORT}`);
  console.log(`✅ Health: http://localhost:${PORT}/api/health`);
  console.log(`🧠 AI Suggestions: http://localhost:${PORT}/api/fast-suggestions`);
  console.log(`🎯 Wireframes: http://localhost:${PORT}/api/generate-wireframe`);
  console.log(`📱 Your frontend will connect automatically!`);
});
